...
void CEpollServer::Run()
{
    ...
    while(true)
    {
        ...
        for(int i = 0; i < nfds; ++i)
        {
            ...
            {   
                rev_size = recv(events[i].data.fd, buffer, MAX_BUFFER_SIZE, 0);
				 printf("rev_size:%d\n",rev_size);
				if(rev_size > 0)
				{
					memcpy(sendbuff, buffer, rev_size);
				}
                if(rev_size == 0)
                {
                ...
                {
                    printf("rev_size:%d,RecvData:\n", rev_size);
		    		PrintData(buffer, rev_size);
                    struct epoll_event ev;
                    ev.events = EPOLLOUT;
                    ev.data.fd = client_socket;
             ...
             else if(events[i].events & EPOLLOUT)
             {
                ...
                //int sendsize = send(client_socket, sendbuff, strlen(sendbuff), MSG_NOSIGNAL);
				int sendsize = send(client_socket, sendbuff,rev_size, MSG_NOSIGNAL);
				 printf("send:sendsbize:%d\n", sendsize);
